...
export function CatalogFiltersBarV2({
  ...
  const activeCount = useMemo(() => [selectedType, selectedFamily].filter(Boolean).length, [selectedType, selectedFamily]);
  ...
  return (
    <nav
      className="sticky top-0 z-30 border-b border-border/70 bg-card/95 shadow-[0_1px_0_rgba(0,0,0,0.03)] backdrop-blur"
        ...
        <div className="space-y-2.5">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            <span className="font-medium uppercase tracking-[0.16em]">
              {activeCount > 0 ? "Filtros ativos" : "Sem filtros"}
            </span>
            {selectedType ? (
            ...
            {activeCount === 0 ? <span>{totalProducts} itens disponíveis</span> : null}
          </div>
        </div>
      </div>
          ...
          <div className="flex h-full flex-col">
            <div className="shrink-0 px-6 pt-6">
              <SheetHeader className="pr-8">
                <SheetTitle>Filtro secundário</SheetTitle>
                <SheetDescription>
                  Tipos organizam a vitrine principal. Famílias refinam a seleção sem ocupar espaço na primeira dobra.
                </SheetDescription>
              </SheetHeader>
            </div>
              ...
              <div className="space-y-5">
                <div className="rounded-2xl border border-border/70 bg-muted/20 p-3">
                  <div className="mb-2 flex items-center justify-between gap-2">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Nível 2</p>
                      <p className="text-sm text-foreground">Famílias em destaque</p>
                    </div>
